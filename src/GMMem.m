...
if isfield(opts,'pars')
	...
	m = X(:,randperm(N, H)); % initialise the centres to random datapoints.
	s2 = mean(diag(cov(X'))); % Take the mean of dimension variances.
...
for emloop = 1:opts.maxit
	...
	loglik = logl(emloop);
	if isfield(opts, 'minChange') && emloop > 1 &&...
			opts.minChange > logl(emloop) - logl(emloop-1)
		minChangeCounter = minChangeCounter + 1;
		if isfield(opts, 'minChangeCount')
			if minChangeCounter > opts.minChangeCount
				break
			end
		else
			break
